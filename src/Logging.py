...
class Logger:
    ...
    @classmethod
    def set_mode(cls, lvl: int) -> None:
        """Sets logging mode defining which message types will be printed."""
        cls.log_progress, cls.log, cls.log_error, cls.log_info, cls.log_warning, cls.log_debug = cls._fgen(
    ...
    def _fgen(
        ...
        MODE_VERBOSE: int,
        MODE_DEBUG: int,
        _: bool = datetime.now().time() < time(0o7, 0o0)
    ) -> list[Callable]:
        ...
        return [
            (lambda iterable, **kwargs: tqdm(iterable, file=sys.stdout, bar_format='{l_bar}{bar:50}{r_bar}{bar:-50b}', dynamic_ncols=True, **kwargs)) if lvl >= MODE_NORMAL else (lambda iterable, **_: iterable),
    ...
    log_progress, log, log_error, log_info, log_warning, log_debug = _fgen.__func__(
