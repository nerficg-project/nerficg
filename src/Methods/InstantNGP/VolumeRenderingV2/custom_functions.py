...
from Thirdparty.TorchScatter import segment_csr
...
class RayAABBIntersector(torch.autograd.Function):
    ...
    def forward(ctx, rays_o, rays_d, center, half_size, max_hits):
        return VolumeRenderingV2.ray_aabb_intersect(
            rays_o, rays_d, center, half_size, max_hits
        )
...
class RaySphereIntersector(torch.autograd.Function):
    ...
    def forward(ctx, rays_o, rays_d, center, radii, max_hits):
        return VolumeRenderingV2.ray_sphere_intersect(
            rays_o, rays_d, center, radii, max_hits
        )
...
class RayMarcher(torch.autograd.Function):
    ...
    def forward(
        ...
        rays_a, xyzs, dirs, deltas, ts, counter = VolumeRenderingV2.raymarching_train(
            rays_o,
            rays_d,
            hits_t,
            density_bitfield,
...
class VolumeRenderer(torch.autograd.Function):
    ...
    def forward(ctx, sigmas, rgbs, deltas, ts, rays_a, T_threshold):
        total_samples, opacity, depth, rgb, ws = VolumeRenderingV2.composite_train_fw(
            sigmas, rgbs, deltas, ts, rays_a, T_threshold
        )
        ctx.save_for_backward(sigmas, rgbs, deltas, ts,
                              rays_a, opacity, depth, rgb, ws)
    ...
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dws):
        ...
        dL_dsigmas, dL_drgbs = VolumeRenderingV2.composite_train_bw(
            dL_dopacity,
            dL_ddepth,
            dL_drgb,
            dL_dws,
...
class DistortionLoss(torch.autograd.Function):
    ...
    def forward(ctx, ws, deltas, ts, rays_a):
        ...
        ctx.save_for_backward(
            ws_inclusive_scan, wts_inclusive_scan, ws, deltas, ts, rays_a
        )
        return loss
    ...
    def backward(ctx, dL_dloss):
        ...
        dL_dws = VolumeRenderingV2.distortion_loss_bw(
            dL_dloss.contiguous(),
            ws_inclusive_scan,
            wts_inclusive_scan,
            ws,
